...
export default function SubscriptionForm({ categories, consentText, consentTextVersion }: { categories: Category[]; consentText: string; consentTextVersion: string }) {
  ...
  return <form onSubmit={submit}><label htmlFor="number">Número internacional de WhatsApp</label><input id="number" value={number} onChange={(event) => setNumber(event.target.value)} placeholder="+56912345678" required /><fieldset><legend>Categorías</legend>{categories.map((category) => <label key={category.slug}><input type="checkbox" checked={selected.includes(category.slug)} onChange={(event) => setSelected(event.target.checked ? [...selected, category.slug] : selected.filter((slug) => slug !== category.slug))} /> {category.name}</label>)}</fieldset><p>{consentText}</p><label><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} required /> Acepto explícitamente este consentimiento ({consentTextVersion}).</label><button type="submit">Suscribirme</button>{message && <p className={message.startsWith("Suscripción") ? "success" : "error"} role="status">{message}</p>}</form>;
}
