...
import { availableGroqSteps, CHAT_PROVIDER_CHAIN, STT_PROVIDER_CHAIN } from './providerChains';
import { PROVIDER_ERROR_CATEGORY, PROVIDER_STATUS, type ChatRequest, type ChatResult, type GroqProviderConfig, type ProviderAttempt, type ProviderStep, type TerminalProviderError, type TranscriptionRequest, type TranscriptionResult } from './types';

interface ErrorWithAttempt extends Error { providerAttempt?: ProviderAttempt; }

function missingAttempt(step: ProviderStep): ProviderAttempt { return { step: step.id, provider: step.id, capability: step.capability, status: PROVIDER_STATUS.SKIPPED, code: 'GROQ_KEY_NOT_CONFIGURED', retryable: false, category: PROVIDER_ERROR_CATEGORY.AUTH, message: 'Provider key not configured' }; }
