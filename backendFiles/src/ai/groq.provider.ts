import { cleanFlatText, requireNonEmptyText } from '../utils/textCleaner';
import { fetchRetryable, type FetchFunction } from './http';
import { AI_CAPABILITY, PROVIDER_ERROR_CATEGORY, PROVIDER_STATUS, type ChatRequest, type ChatResult, type GroqProviderConfig, type ProviderAttempt, type ProviderErrorCategory, type ProviderStep, type TranscriptionRequest, type TranscriptionResult } from './types';
...
function attemptFromError(step: ProviderStep, error: unknown): ProviderAttempt {
  ...
  return { step: step.id, provider: step.id, capability: step.capability, status: PROVIDER_STATUS.FAILED, code: info.code, httpStatus: info.httpStatus, retryable: info.retryable, category: info.category, message: info.message };
}
...
export class GroqProvider {
  ...
  public async transcribe(step: ProviderStep, request: TranscriptionRequest): Promise<TranscriptionResult> {
    ...
    try {
      ...
      const blob = new Blob([request.audio], { type: request.mimeType ?? 'audio/m4a' });
      ...
      return { text: requireNonEmptyText(body.text ?? '', 'transcript'), provider: step.id, model: step.model, attempts: [{ step: step.id, provider: step.id, capability: step.capability, status: PROVIDER_STATUS.SUCCESS, retryable: false, message: 'ok' }] };
  ...
  public async chat(step: ProviderStep, request: ChatRequest): Promise<ChatResult> {
    ...
    try {
      ...
      return { reply: requireNonEmptyText(body.choices?.[0]?.message?.content ?? '', 'assistant reply'), provider: step.id, model: step.model, attempts: [{ step: step.id, provider: step.id, capability: step.capability, status: PROVIDER_STATUS.SUCCESS, retryable: false, message: 'ok' }] };
